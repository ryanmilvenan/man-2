...
module.exports = function(socket) {
    ...
    if(handshakeData) {
        console.log(util.inspect(handshakeData, false, null));
    ...
    if(handshakeData.sessionID) {
        session = sessions[handshakeData.sessionID];
    }
    ...
    socket.on('sources:retrieve', function(data) {
        ...
        var sid = handshakeData.headers.cookie.replace(/.*connect.sid=(.*)?.*/, '$1');
        handshakeData.sessionID = sid.split('.')[0].substring(4); 
        db.Session.findOne({'session': handshakeData.sessionID }, function(err, sessDoc) {
            if(err) console.error(err);
            if(sessDoc) {
                var jsonSession = JSON.parse(sessDoc.session);
                var sessUser = jsonSession.user;
                if(sessUser) {
                    db.User.findOne({'username': sessUser}, 'tabs', function(err, doc) {
                        if(err) return console.error(err);
                        console.log(doc);
                      socket.emit('server:tabs', {tabs: doc.tabs});
                    });
                } else {
                    db.User.findOne({'username':'default'}, 'tabs', function(err, doc) {
                      socket.emit('server:tabs', {tabs: doc.tabs});
                    });
                }
            } else {
                    ...
                    if(isMatch) {
                        ...
                        if(session) {
                            ...
                            session.save(function(err) {
                                if(err) return console.err(err);
                                db.Session.findOne({'session': handshakeData.sessionID}, 'session', function(err, doc) {
                                    if(err) console.error(err);
                                    if(doc) {
